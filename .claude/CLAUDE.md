...
- **SessionEnd hooks** — sync hook with fast exit. Prints informational message (visible to user), then spawns a detached worker (`nohup ... </dev/null &>/dev/null & disown`) for Phase 0 (checkpoint commit) + LLM calls (`claude -p --model haiku --max-budget-usd 0.15 --system-prompt "" --tools ""`). PID lockfile (`~/.tandem/state/.worker.lock`) prevents overlapping workers.
- **`claude -p` budget floor** — `claude -p` has a base cost of ~$0.06 even with empty system prompt. All background LLM calls must use `--max-budget-usd 0.15` minimum. Use `--system-prompt "" --tools ""` to strip context overhead (CLAUDE.md, MCP servers, skills). Budget exceeded returns exit 0 with "Error: Exceeded USD budget" on stdout, so always check for `Error:*` prefix.
