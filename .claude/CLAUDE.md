...
- **Auto-memory directory** — computed from CWD: `~/.claude/projects/$(echo "$CWD" | sed 's|/|-|g')/memory/`. This matches Claude Code's native convention. Every script that touches progress.md or MEMORY.md uses this pattern. Memory is strictly isolated per-project (no cross-contamination). Only `~/.tandem/memory/global.md` is shared (metadata-only session recaps). CWD that is not a git root triggers a startup warning since memory may not be project-scoped.
